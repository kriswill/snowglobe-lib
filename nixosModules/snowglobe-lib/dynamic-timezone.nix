...
let
  cfg = config.snowglobe-lib.dynamic-timezone;
    ...
    warnings =
      lib.optionals (!config.networking.networkmanager.enable) [
        ''
          ...
          You can enable networkmanager with networking.networkmanager.enable = true or disable the module with snowglobe-lib.dynamic-timezone.enable = false.
        ''
      ]
      ++ (lib.optionals (config.time.timeZone != null) [
        ''
          the time.timeZone option must be `null` for the snowglobe-lib.dynamic-timezone module to work.
        ''
      ]);
